...
export class PaginaTicketsComunidad {
  ...
  readonly menuItemTicketsComunidad: Locator;
  ...
  constructor(page: Page) {
    ...
    this.menuItemTicketsComunidad = page.getByRole('menuitem', { name: 'Tickets comunidad' });
  }

  async abrirDesdeHeader(header: PaginaHeader) {
    void header;
    await this.page.goto('/dashboard');
    await this.page.getByRole('button', { name: /account of current user/i }).click();
    await this.menuItemTicketsComunidad.waitFor({ state: 'visible' });
    await this.menuItemTicketsComunidad.click();
